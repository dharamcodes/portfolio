      ...
      <div className={styles["experience-tab-container"]}>
        <div className={styles["experience-tabs"]} role="tablist" aria-label="Experience tabs">
          {experience.map((exp, i) => (
            <button
              key={i}
              className={`${styles["experience-tab-btn"]} ${i === activeExpIndex ? styles["experience-tab-btn-active"] : ""}`}
            ...
            >
              ...
              <span className={styles["exp-tab-period"]}>
                {exp.duration.split(" – ")[1] ?? exp.duration}
              </span>
            </button>
          ))}
        </div>
        ...
        <div
          ...
          className={`scrollable-content ${styles["experience-details-wrapper"]} glass`}
        >
          {experience.map((exp, i) => {
                ...
                <div className={styles["exp-detail-header"]}>
                  <div>
                    <h3 className={styles["exp-detail-role"]}>
                      {exp.role} <span className="gradient-text">@ {exp.company}</span>
                    </h3>
